void main()
{
    ...
    {
      fragmentColor += GetColorAfterDirectionalLight(directionalLights[i], vec3(Position), normalize(vec3(Normal)));
    }
    ...
    {
      fragmentColor += GetColorAfterSpotLight(spotLights[i], vec3(Position), normalize(vec3(Normal)));
    }
