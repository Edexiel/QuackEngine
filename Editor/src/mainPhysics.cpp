...
// reactphysics3d
    ...
    reactphysics3d::Vector3 positionSphere{0,10,10};
    ...
    reactphysics3d::BoxShape* cubeShape = physicsCommon.createBoxShape({scaleCube.x, scaleCube.y, scaleCube.z});

    reactphysics3d::Collider* colliderSphere = rbSphere->addCollider(sphereShape, reactphysics3d::Transform::identity());
    reactphysics3d::Collider* colliderCube = rbCube->addCollider(cubeShape, reactphysics3d::Transform::identity());

    reactphysics3d::Material& materialCube = colliderCube->getMaterial();
    materialCube.setBounciness(1.0f);

    while (!glfwWindowShouldClose(window))
    {
      world->update(1.f/144);
      ...
      //std::cout << "Sphere : X = " << ts.getPosition().x << ", Y = " << ts.getPosition().y << ",Z = " << ts.getPosition().z << std::endl;
      //std::cout << "Cube : X = " << tc.getPosition().x << ", Y = " << tc.getPosition().y << ",Z = " << tc.getPosition().z << std::endl;
