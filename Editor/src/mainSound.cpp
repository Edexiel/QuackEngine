...
{
    int argc = 3;
    char* argv[2] {  "../../../dragonball-z-abridged-space-duck.wav", "../../../mac-quack.mp3"};
    ...
    for (iDecoder = 0; iDecoder < g_decoderCount; ++iDecoder) {
        result = ma_decoder_init_file(argv[0+iDecoder], &decoderConfig, &g_pDecoders[iDecoder]);
  ...
  //printf("Press enter to relaunch");
  //getchar();

  ma_decoder_seek_to_pcm_frame(&g_pDecoders[0], 0);
  ma_decoder_seek_to_pcm_frame(&g_pDecoders[1], 0);
