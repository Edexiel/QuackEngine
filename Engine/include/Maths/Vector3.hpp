...
template <typename T> inline T Vector3<T>::SqrLength() const {
  return x * x + y * y + z * z;
}
...
template <typename T> inline void Vector3<T>::Normalize(Vector3<T> &v) {
  T length = v.Length();
...
template <typename T> inline Vector3<T> Vector3<T>::Up() { return {0, 1, 0}; }

template <typename T> inline Vector3<T> Vector3<T>::Down() {
  return {0, -1, 0};
}

template <typename T> inline Vector3<T> Vector3<T>::Right() {
  return {1, 0, 0};
}

template <typename T> inline Vector3<T> Vector3<T>::Left() {
  return {-1, 0, 0};
}

template <typename T> inline Vector3<T> Vector3<T>::Forward() {
  return {0, 0, 1};
}

template <typename T> inline Vector3<T> Vector3<T>::Backward() {
  return {0, 0, -1};
}

template <typename T> inline Vector3<T> Vector3<T>::Zero() { return {0, 0, 0}; }
...
template <typename T> Vector3<T> Vector3<T>::One() { return {1, 1, 1}; }
