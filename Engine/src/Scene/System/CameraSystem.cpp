...
void CameraSystem::FreeFly()
{
    ...
    transform.forward = (transform.rotation.ToMatrix() * Vector4f(Vector3f::Backward(), 0)).xyz * cam._scaleAxisZ;
    transform.right = (transform.rotation.ToMatrix() * Vector4f(Vector3f::Left(), 0)).xyz * cam._scaleAxisX;
